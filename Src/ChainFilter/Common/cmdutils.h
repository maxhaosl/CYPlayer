...
/**
 * Print an error message to stderr, indicating pszFileName and a human
 ...
 * @see av_strerror()
 */
static inline void print_error(const char *pszFileName, int err)
{
    char szTmp[AV_ERROR_MAX_STRING_SIZE] = { 0 };
    av_log(NULL, AV_LOG_ERROR, "%s: %s\n", pszFileName, av_make_error_string(szTmp, AV_ERROR_MAX_STRING_SIZE, err));
}
 ...
 *
 * @param pszFileName buffer where the name of the found pszFileName is written
 * @param filename_size size in bytes of the pszFileName buffer
 * @param preset_name name of the preset to search
 * @param is_path tell if preset_name is a pszFileName path
 * @param codec_name name of the codec for which to look for the
 * preset, may be NULL
 */
FILE *get_preset_file(char *pszFileName, size_t filename_size,
...
char *file_read(const char *pszFileName);
