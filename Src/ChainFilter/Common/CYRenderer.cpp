...
typedef struct RendererContext {
    ...
    // Can be nullptr when vulkan instance is created by avutil
...
{
    AVDictionaryEntry *entry = av_dict_get(opt, "debug", nullptr, 0);
    int debug = entry && strtol(entry->value, nullptr, 10);
    return debug;
}
                                  ...
                                  AVDictionary **dict)
{
    ...
    entry = av_dict_get(opt, inst_ext_key, nullptr, 0);
                                ...
                                AVDictionary **dict)
{
    ...
    entry = av_dict_get(opt, dev_ext_key, nullptr, 0);
    if (entry && entry->value && entry->value[0])
        av_bprintf(&buf, "+%s", entry->value);
...
static const char *select_device(const AVDictionary *opt)
{
    ...
    entry = av_dict_get(opt, "device", nullptr, 0);
    if (entry)
        return entry->value;
    return nullptr;
}
...
{
    RendererContext *ctx = (RendererContext *) renderer;
    VkQueueFamilyProperties *queue_family_prop = nullptr;
...
static int create(VkRenderer *renderer, SDL_Window *window, AVDictionary *opt)
{
    ...
    if (!SDL_Vulkan_GetInstanceExtensions(window, &num_ext, nullptr)) {
        av_log(nullptr, AV_LOG_FATAL, "Failed to get vulkan extensions: %s\n",
               SDL_GetError());
        return AVERROR_EXTERNAL;
    }
    ...
    entry = av_dict_get(opt, "create_by_placebo", nullptr, 0);
    if (entry && strtol(entry->value, nullptr, 10))
...
static int create_hw_frame(VkRenderer *renderer, AVFrame *frame)
{
    ...
    if (ctx->hw_frame_ref) {
        ...
        if (hw_frame->width == pFrame->width &&
            hw_frame->height == pFrame->height &&
    ...
    if ((ctx->constraints->max_width &&
         ctx->constraints->max_width < pFrame->width) ||
        (ctx->constraints->max_height &&
         ctx->constraints->max_height < pFrame->height) ||
        (ctx->constraints->min_width &&
         ctx->constraints->min_width > pFrame->width) ||
        (ctx->constraints->min_height &&
         ctx->constraints->min_height > pFrame->height))
        return 0;
...
static inline int check_hw_transfer(RendererContext *ctx, AVFrame *frame)
{
    ...
    for (int i = 0; ctx->transfer_formats[i] != AV_PIX_FMT_NONE; i++)
        if (ctx->transfer_formats[i] == pFrame->format)
            return 1;
...
static int map_frame(VkRenderer *renderer, AVFrame *frame, int use_hw_frame)
{
    ...
    if (ret != AVERROR(ENOSYS))
        av_log(nullptr, AV_LOG_FATAL, "Map frame failed: %s\n", av_err2str(ret));
    return ret;
}
...
static int transfer_frame(VkRenderer *renderer, AVFrame *frame, int use_hw_frame)
{
    ...
    if (ret != AVERROR(ENOSYS))
        av_log(nullptr, AV_LOG_FATAL, "Transfer frame failed: %s\n",
               av_err2str(ret));
    return ret;
}
...
static int convert_frame(VkRenderer *renderer, AVFrame *frame)
{
    ...
    if (pFrame->format == AV_PIX_FMT_VULKAN)
        return 0;
...
static int display(VkRenderer *renderer, AVFrame *frame)
{
    ...
    if (!pl_map_avframe_ex(ctx->placebo_vulkan->gpu, &pl_frame, pl_avframe_params(
            .frame = frame,
            .pTex = ctx->pTex))) {
        av_log(nullptr, AV_LOG_ERROR, "pl_map_avframe_ex failed\n");
        return AVERROR_EXTERNAL;
    }

    if (!pl_swapchain_start_frame(ctx->swapchain, &swap_frame)) {
        av_log(nullptr, AV_LOG_ERROR, "start frame failed\n");
        ret = AVERROR_EXTERNAL;
        goto out;
    }
    ...
    if (!pl_render_image(ctx->renderer, &pl_frame, &target,
        ...
        av_log(nullptr, AV_LOG_ERROR, "pl_render_image failed\n");
        ret = AVERROR_EXTERNAL;
        goto out;
    }

    if (!pl_swapchain_submit_frame(ctx->swapchain)) {
        av_log(nullptr, AV_LOG_ERROR, "pl_swapchain_submit_frame failed\n");
        ret = AVERROR_EXTERNAL;
        goto out;
    }
...
static void destroy(VkRenderer *renderer)
{
    ...
    if (ctx->placebo_vulkan) {
        for (int i = 0; i < FF_ARRAY_ELEMS(ctx->pTex); i++)
            pl_tex_destroy(ctx->placebo_vulkan->gpu, &ctx->pTex[i]);
    ...
    if (ctx->vk_surface) {
        ...
        vkDestroySurfaceKHR(ctx->inst, ctx->vk_surface, nullptr);
