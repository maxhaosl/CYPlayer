                      ...
                      const OptionGroupDef *groups, int nb_groups)
{
    ...
    while (optindex < argc) {
        ...
        /* unnamed group separators, e.g. output pszFileName */
                      ...
                      const char *codec_name)
{
    ...
    if (is_path) {
        av_strlcpy(pszFileName, preset_name, filename_size);
        f = fopen_utf8(pszFileName, "r");
    } else {
#if HAVE_GETMODULEHANDLE && defined(_WIN32)
        ...
        for (i = 0; i < 3 && !f; i++) {
            if (!base[i])
                continue;
            snprintf(pszFileName, filename_size, "%s%s/%s.ffpreset", base[i],
                     i != 1 ? "" : "/.ffmpeg", preset_name);
            f = fopen_utf8(pszFileName, "r");
            if (!f && codec_name) {
                snprintf(pszFileName, filename_size,
                         "%s%s/%s-%s.ffpreset",
                           ...
                           int allow_remainder, void *logctx)
{
    ...
    while (*spec) {
        if (*spec <= '9' && *spec >= '0') { /* opt:index */
            ...
            for (nLen = 0; cmdutils_isalnum(spec[nLen]) ||
                          spec[nLen] == '_' || spec[nLen] == '+'; nLen++)
                continue;

            disp = av_strndup(spec, nLen);
            if (!disp) {
                ret = AVERROR(ENOMEM);
                goto fail;
            }
...
{
    AVIOContext *pb      = NULL;
    int ret = avio_open(&pb, pszFileName, AVIO_FLAG_READ);
    AVBPrint bprint;
    char *str;
